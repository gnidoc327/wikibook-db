...
async def init_db(tmp_path_factory, worker_id):
    ...
    from ch01.main import _create_master_admin

    base = tmp_path_factory.getbasetemp().parent  # 모든 xdist 워커가 공유하는 경로
    lock_path = str(base / "ch01_init.lock")
    done_path = base / "ch01_init.done"
...
async def admin_headers(init_db) -> dict:
    ...
    from ch01.dependencies.auth import create_access_token
...
async def _test_conn(init_db):
    ...
    """
    from ch01.dependencies.mysql import _engine, get_session
    from ch01.main import app
